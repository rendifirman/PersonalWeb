...
@section('description', 'Kelola daftar pengalaman yang tampil di halaman utama.')
...
<div class="space-y-8">
    ...
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-indigo-50 via-white to-purple-50 p-6 border border-indigo-100 animate-fade-in-up">
        <div class="relative z-10 flex flex-wrap items-center justify-between gap-4">
            <div class="flex items-center gap-2">
                ...
                <div>
                    <h1 class="text-xl font-bold text-gray-900">Kelola Pengalaman</h1>
                    <p class="text-gray-600 text-sm mt-0.5">Kelola pengalaman kerja dan proyek sebelumnya.</p>
                </div>
            </div>
            <a href="{{ route('admin.experiences.create') }}" class="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-indigo-600 to-purple-600 px-5 py-2.5 text-white font-medium shadow-sm hover:shadow-md transition-all hover:-translate-y-0.5">
    ...
    <div class="rounded-2xl bg-white shadow-sm border border-gray-100 overflow-hidden animate-fade-in-up" style="animation-delay: 0.05s">
            ...
            <table class="w-full table-hover">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Perusahaan</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($experiences as $experience)
                        <tr class="transition">
                            ...
                            <td class="px-6 py-4 text-gray-600">{{ $experience->period }}</td>
                            <td class="px-6 py-4">
                                @if($experience->show_on_homepage)
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.experiences.edit', $experience) }}" class="inline-flex items-center gap-1 rounded-full border border-gray-300 bg-white px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50 transition shadow-sm">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                        Edit
                                    </a>
                                    <form action="{{ route('admin.experiences.destroy', $experience) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus pengalaman ini?')" class="inline">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                ...
                                <div class="mt-3">
                                    <a href="{{ route('admin.experiences.create') }}" class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">Tambah pengalaman sekarang →</a>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        @if(isset($experiences) && method_exists($experiences, 'hasPages') && $experiences->hasPages())
